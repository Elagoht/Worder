...
db.regexQueries([r"n$",r"^F"],[[0],[1]],["i","si"])"""
		if columns==[]: columns=[[] for _ in regexes]
		result=self
		for regex,column,flag in zip(regexes,columns,flags): result=result.regexQuery(regex,column,flag)
		return result
	...
	def extend(self,count=1):
		...
		for _ in range(count):
			self.add(*[None for _ in range(self.columnCount)])
		self.transpose()
...
if columns parameter is [], it applies to all columns."""
		if len(self):
			...
			for row in range(len(data)):
				for col in columns:
					data[row][col]=self.__convert__(function(data[row][col],row))
